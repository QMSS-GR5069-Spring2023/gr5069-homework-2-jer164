...
donor_cleaner <- function(input_data_path, decision_maker = "default", state_fin, is_zip = FALSE){
  ...
  if (state_fin == "NC"){
    ...
    temp_data <- read.csv(input_data_path, skip = 1) %>% as_tibble()
  ...
  total_cols <- length(colnames(temp_data))
  
  print(glue("Columns: {total_cols}"))
  print(colnames(temp_data))
  
  
  write.csv(temp_data, glue("E:/Documents/Data_Projects/donoRs/output/{decision_maker}_donors.csv"), 
            row.names = FALSE, na = "")
